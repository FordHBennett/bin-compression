...
void Huffman::encode(std::string filename) {
    ...
    if (!file) {
        ...
        if (!writeTableFile(filename)) return; // <-- Corrected here
...
void Huffman::decode(std::string data_filename, std::string table_filename) {
    char charbuffer;
    int buffer;  // <-- Changed from char buffer[256] to int
    ...
    size = 0;
    for (it = encodedResult.begin(); it != encodedResult.end(); it++) size += it->second;
    ...
    std::ofstream file("decoded.bin", std::ios::binary);
    file.write(datafile->data, datafile->size);
    file.close();
}
...
void Huffman::createDataFile(intpair_v& v) {
    ...
    datafile->data[charpos] = (datafile->data[charpos]) | (bit << charbitpos); // <-- Parentheses to ensure correct order
	...
	if (!file) {
        ...
        // Read the table entries
            while (!file.eof()) {
                ...
                file.read(reinterpret_cast<char*>(&buffer), 4);  // <-- Correct conversion from buffer to integer
                temp.bits = buffer;
    			if (file.fail()) throw "Error reading tablefile";
...
bool Huffman::writeTableFile(std::string fstr) {
    ...
    if (!file) {
        ...
        for (it = tablefile->data.begin(); it != tablefile->data.end(); it++) {
            ...
            file.write(reinterpret_cast<char*>(&it->size), 1);  // <-- Correct conversion from int to char*
            file.write(reinterpret_cast<char*>(&it->bits), 4);  // <-- Correct conversion from int to char*
    }
    }
    file.close();
    return true;
