...
void ShannonFano::Write_Binary_Frequencies_Per_Row_To_Json_File(const std::filesystem::path& binary_path, const std::filesystem::path& json_path, const int& row_number, const int& number_of_bytes_to_read) const{
    std::vector<char> binary_data_vec;
...
if (input_file.fail() && !input_file.eof()) {
    ...
    if(!output_file) {
        ERROR_MSG_AND_EXIT("Error: Unable to open the file.");
            ...
            if(byte_frequencies_vec.size() < 15){
                output_file << "{\n";
    ...
    if(!output_file) {
        ERROR_MSG_AND_EXIT("Error: Unable to open the file.");
            ...
            if(byte_pair_frequencies_vec.size() < UINT8_MAX){
                ...
                output_file << "        \"" << binary_path_string << "\": {\n";
                for (auto it = byte_pair_to_index_map.begin(); it != byte_pair_to_index_map.end(); it++) {
                    output_file << "        \"" << static_cast<unsigned int>(static_cast<uint8_t>(it->first >> 8)) << static_cast<unsigned int>(static_cast<uint8_t>(it->first)) << "\": " << it->second;dsaw
                    if (std::next(it) != byte_pair_to_index_map.end()) {
                        output_file << ",\n";
                    } else {
                        output_file << "\n";
